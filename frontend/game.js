...
async function loadMemo() {
  ...
  try {
    ...
    console.error('加载 memo 失败: - game.js:77', e);
    const lang = getCurrentLanguage();
    const errorMessages = {
      zh: '加载失败，请稍后重试',
      en: 'Loading failed, please try again later',
      ja: '読み込みに失敗しました。後で再試行してください'
    };
    memoContent.innerHTML = `<div id="memo-placeholder">${errorMessages[lang] || errorMessages.zh}</div>`;
  }
}
...
// 多语言支持
const STATES = {
  idle: { zh: '待命', en: 'Idle', ja: '待機', area: 'breakroom' },
  writing: { zh: '整理文档', en: 'Writing', ja: '文書整理', area: 'writing' },
  researching: { zh: '搜索信息', en: 'Researching', ja: '情報検索', area: 'researching' },
  executing: { zh: '执行任务', en: 'Executing', ja: 'タスク実行', area: 'writing' },
  syncing: { zh: '同步备份', en: 'Syncing', ja: '同期中', area: 'writing' },
  error: { zh: '出错了', en: 'Error', ja: 'エラー', area: 'error' }
};
...
const BUBBLE_TEXTS = {
  ...
  writing: {
    ...
    en: [
      ...
      'Let\'s get the critical path working first',
      'I\'ll simplify the complex',
      'Putting bugs in cages',
  ...
  researching: {
    ...
    en: [
      'Digging for evidence chain',
      'Let me turn information into conclusions',
      ...
      'Don\'t rush, draw a causal diagram first'
    ],
    ja: [
      '証拠チェーンを掘り下げています',
      '情報を結論に変えます',
  ...
  executing: {
    ...
    en: [
      ...
      'Minimum viable first, then beautiful version'
    ],
    ja: [
      '実行中：瞬きしないで',
      'タスクを小さく分割して攻略',
  ...
  syncing: {
    ...
    en: [
      ...
      'Backup isn\'t a ritual, it\'s security',
      'Writing… don\'t cut the power',
      'Let timestamps handle changes',
  ...
  error: {
    ...
    en: [
      ...
      'I\'m on it: Locating root cause now',
...
function getCurrentLanguage() {
  if (typeof window !== 'undefined' && window.uiLang) {
    return window.uiLang;
  }
  return 'zh'; // 默认中文
}
...
function getStateName(state) {
  ...
  return stateInfo ? stateInfo[lang] || stateInfo.zh : state;
}

// 获取气泡文本
function getBubbleTexts(state) {
  const lang = getCurrentLanguage();
  const texts = BUBBLE_TEXTS[state];
  return texts ? texts[lang] || texts.zh : [];
}
...
async function initGame() {
  ...
  console.log('WebP 支持: - game.js:479', supportsWebP);
...
function updateGameState(state) {
  ...
  } else if (effectiveState === 'error' || effectiveState === 'syncing') {
    sofa.anims.stop();
...
function update(time) {
  ...
  updateGameState(effectiveStateForServer);

  // 错误bug移动逻辑
  if (window.errorBug && effectiveStateForServer === 'error') {
...
function fetchStatus() {
  fetch('/status')
    ...
    .then(data => {
      ...
      const stateName = getStateName(nextState);
      const changed = (pendingDesiredState === null) && (nextState !== currentState);
      const nextLine = '[' + stateName + '] ' + (data.detail || '...');
      if (changed) {
    ...
    .catch(error => {
      console.error('获取状态失败: - game.js:981', error);
      const lang = getCurrentLanguage();
      const errorMessages = {
        zh: '连接失败，正在重试...',
      ...
      typewriterTarget = errorMessages[lang] || errorMessages.zh;
      typewriterText = '';
      typewriterIndex = 0;
    });
}
...
function showBubble() {
  ...
  const texts = getBubbleTexts(currentState);
  if (currentState === 'idle' || texts.length === 0) return;
...
function showCatBubble() {
  ...
  const texts = getBubbleTexts('cat');
...
function fetchAgents() {
  fetch('/agents?t=' + Date.now(), { cache: 'no-store' })
    ...
    .catch(error => {
      console.error('拉取 agents 失败: - game.js:1147', error);
      // 可以在这里添加用户友好的错误提示，例如显示一个临时的错误消息
    });
}
...
function renderAgent(agent) {
  ...
  if (!agents[agentId]) {
    ...
    container.on('pointerdown', () => showAgentDetails(agent));
...
function showAgentDetails(agent) {
  // 先销毁之前的详细信息面板
  if (window.agentDetailsPanel) {
    window.agentDetailsPanel.destroy();
    window.agentDetailsPanel = null;
  }
  
  const lang = getCurrentLanguage();
  const stateName = getStateName(agent.state || 'idle');
  
  // 认证状态翻译
  const authStatusTranslations = {
    approved: { zh: '已批准', en: 'Approved', ja: '承認済み' },
    pending: { zh: '待批准', en: 'Pending', ja: '承認待ち' },
    rejected: { zh: '已拒绝', en: 'Rejected', ja: '拒否済み' },
    offline: { zh: '离线', en: 'Offline', ja: 'オフライン' }
  };
  
  const authStatusText = authStatusTranslations[agent.authStatus] ? 
    authStatusTranslations[agent.authStatus][lang] : 
    agent.authStatus || '未知';
  
  // 区域翻译
  const areaTranslations = {
    breakroom: { zh: '休息区', en: 'Breakroom', ja: '休憩エリア' },
    writing: { zh: '工作区', en: 'Writing', ja: '作業エリア' },
    error: { zh: '错误区', en: 'Error', ja: 'エラーエリア' }
  };
  
  const areaText = areaTranslations[agent.area] ? 
    areaTranslations[agent.area][lang] : 
    agent.area || '未知';
  ...
  // 面板背景
  const background = game.add.rectangle(0, 0, panelWidth, panelHeight, 0x1a1a2e, 0.95);
  background.setStrokeStyle(2, 0x64477d);
  
  // 标题
  const title = game.add.text(0, -70, agent.name || 'Agent', {
    fontFamily: 'ArkPixel, monospace',
    fontSize: '16px',
    fill: '#ffd700',
    stroke: '#000',
  ...
  // 状态信息
  const stateText = game.add.text(0, -40, `${lang === 'zh' ? '状态: ' : 'State: '}${stateName}`, {
    fontFamily: 'ArkPixel, monospace',
    fontSize: '12px',
    fill: '#cbd5e1'
  }).setOrigin(0.5);
  ...
  // 认证状态
  const authText = game.add.text(0, -15, `${lang === 'zh' ? '认证: ' : 'Auth: '}${authStatusText}`, {
    fontFamily: 'ArkPixel, monospace',
    fontSize: '12px',
    fill: '#cbd5e1'
  }).setOrigin(0.5);
  ...
  // 区域信息
  const areaInfoText = game.add.text(0, 10, `${lang === 'zh' ? '区域: ' : 'Area: '}${areaText}`, {
    fontFamily: 'ArkPixel, monospace',
    fontSize: '12px',
    fill: '#cbd5e1'
  }).setOrigin(0.5);
  ...
  // 详情信息
  const detailText = game.add.text(0, 35, `${lang === 'zh' ? '详情: ' : 'Detail: '}${agent.detail || '-'}`.substring(0, 30), {
    fontFamily: 'ArkPixel, monospace',
    fontSize: '10px',
    fill: '#cbd5e1',
  ...
  // 关闭按钮
  const closeButton = game.add.text(panelWidth/2 - 15, panelHeight/2 - 15, '✕', {
    fontFamily: 'ArkPixel, monospace',
    fontSize: '14px',
    fill: '#e94560',
    stroke: '#000',
  ...
  container.add([background, title, stateText, authText, areaInfoText, detailText, closeButton]);
  container.setDepth(2500);
  
  window.agentDetailsPanel = container;
}
